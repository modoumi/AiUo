...
namespace AiUo.Common;

/// <summary>
/// 数制格式化器，支持进制互转，中文，货币格式
/// </summary>
public class NumberFormater
{
    ...
    public NumberFormater(int radix = 62)
    {
        if (radix < 2)
        {
            radix = 2;
        ...
        Radix = radix;
        Characters = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ+/".Substring(0, radix);
    }
    ...
    public string ToString(BigInteger number)
    {
        ...
        {
            var mod = t % Radix;
            t = BigInteger.Abs(BigInteger.Divide(t, Radix));
    ...
    {
        int j = 0;
        return new string(str.ToCharArray().Reverse().ToArray()).Where(ch => Characters.Contains(ch)).Sum(ch => Characters.IndexOf(ch) * (long)Math.Pow(Radix, j++));
    }
    ...
    public BigInteger FromStringBig(string str)
    {
        ...
        return chars.Aggregate(BigInteger.Zero, (current, c) => current + Characters.IndexOf(c) * BigInteger.Pow(Radix, j++));
    }
    ...
    {
        return Radix + "进制模式，进制符：" + Characters;
    }
